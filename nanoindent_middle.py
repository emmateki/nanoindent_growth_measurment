...
def save_pics(grid_final_final, img, folder_name, data_root_path):
    ...
    Args:
        grid_final_final: Processed image data.
    ...
    plt.ioff()
    visualization.draw_grid(img, grid_final_final)
...
def processing(img, folder_name, data_root_path):
    ...
    Args:
        img: Original image data.
        folder_name: Name of the folder where the image is located-number.
        data_root_path: Path to the root data directory.
    """
    N_ROWS=11
    PARTS = 3
    ...
    try:
        ...
        if nan_count >= ((n_rows * n_columns) / 4 * 3):
            ...
            ip.calculate_distance_and_save_big(grid_final_final, folder_name, data_root_path)
